[11:07 PM, 3/20/2026] Aadish (Sumit friend): ### PostgreSQL

sql
-- Drop all application tables (JPA entities + trip_tag join table). CASCADE removes FK deps.

DROP TABLE IF EXISTS trip_tag CASCADE;

DROP TABLE IF EXISTS splitwise_expense_splits CASCADE;
DROP TABLE IF EXISTS splitwise_settlement_expenses CASCADE;
DROP TABLE IF EXISTS splitwise_expenses CASCADE;
DROP TABLE IF EXISTS splitwise_balances CASCADE;
DROP TABLE IF EXISTS splitwise_activities CASCADE;
DROP TABLE IF EXISTS splitwise_settlements CASCADE;
DROP TABLE IF EXISTS splitwise_group_members CASCADE;
DROP TABLE IF EXISTS splitwise_groups CASCADE;

DROP TABLE IF EXISTS message CASCADE;
DROP TABLE IF EXISTS co…
[11:11 PM, 3/20/2026] Aadish (Sumit friend): Here’s what was added and how to use it.

### File
*tranzo-user-ms/sql/create_schema_postgresql.sql*

It contains *CREATE TABLE* statements for the main JPA tables (users, profile, trips, tags, trip_tag, chat, notifications, ratings, travel pals, splitwise, task_lock, etc.), with *primary keys, **uniques, **indexes* from @Table / @Index, and *foreign keys* where the entities use @JoinColumn / @MapsId.

### How to run (PostgreSQL)
bash
psql -U your_user -d your_database -f tranzo-user-ms/sql/create_schema_postgresql.sql


### Notes
1. *Not a byte-for-byte Hibernate dump* — column lengths and a few defaults are chosen to match the entities; after the first run you can compare with Hibernate’s DDL or ddl-auto=validate.
2. *aadhar_otp* — the entity’s @Table(uniqueCo…
[11:12 PM, 3/20/2026] Aadish (Sumit friend): -- Manual DDL aligned with JPA entities in tranzo-user-ms (PostgreSQL).
-- Prefer letting Hibernate create/update schema (application.yaml ddl-auto) or export from JPA.
-- Run after: CREATE EXTENSION IF NOT EXISTS "pgcrypto";  -- for gen_random_uuid()
