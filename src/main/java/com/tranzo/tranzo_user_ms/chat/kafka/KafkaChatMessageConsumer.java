...
public class KafkaChatMessageConsumer {
    ...
    public void onTripPublishEvent(String messageJson){
        try{
            ...
            if ("TRIP_PUBLISHED".equals(event.getEventType())) handleTripPublishChatCreation(event);
            if("PARTICIPANT_JOINED".equals(event.getEventType())) handleParticipantJoinChatRequest(event);
    ...
    private void handleTripPublishChatCreation(ChatTripPublishedEventDto event){
        ...
        kafkaChatMessageProducer.publishGroupChatCreated(conversationEntity,tripId);
    }

    private void handleParticipantJoinChatRequest(ChatTripPublishedEventDto event){
        log.info("Received ParticipantJoinedEvent in chat for tripId={}", event.getTripId());
        UUID hostUserId = event.getHostUserId();
        UUID tripId = event.getTripId();
        ConversationEntity conversationEntity =  createAndManageConversationService.createTripGroupChat(hostUserId);
        kafkaChatMessageProducer.publishGroupChatCreated(conversationEntity,tripId);
