# SQL Schema from JPA Entities

This document contains SQL DDL scripts derived from the entity classes and models in `tranzo-user-ms`. Tables are created with the same names, column mappings, indexes, unique constraints, and foreign key relationships as defined in the JPA entities.

**Target dialect:** PostgreSQL (UUID type, TIMESTAMP, JSONB). Adjust types if using another DB (e.g. MySQL: BINARY(16) for UUID, DATETIME, JSON).
...
```sql
-- Column names FIRST_NAME, GENDER, etc. match entity; quote them in PostgreSQL if case-sensitive.
...
-- Note: Entity declares uniqueConstraint on (user_uuid, document_type) but has only user_id and no document_type.
-- Option A: Add document_type and use uk_aadhar_otp_user_document_type (see alternate block below).
-- Option B: Add CONSTRAINT uk_aadhar_otp_user_reference UNIQUE (user_id, reference_id).
```

If you align the entity with the constraint, use this instead (with `document_type`):
...
CREATE TABLE aadhar_otp (
    ...
    created_at     TIMESTAMP    NOT NULL,
    PRIMARY KEY (id),
    CONSTRAINT uk_aadhar_otp_user_document_type UNIQUE (user_uuid, document_type)
);
```
...
### 2.5 `trip_tag` (join table: TripEntity ↔ TagEntity)
...
## 5. Creation order (for clean run)

To satisfy foreign keys, create tables in this order (or drop constraints, create all tables, then add FKs):

1. `users`
2. `user_profile`, `user_profile_history_table`, `refresh_token`, `verification`, `aadhar_otp`, `social_handle`, `travel_pal`, `user_reports`
3. `core_trip_details`
4. `trip_policies`, `trip_meta_data`, `tags`, `trip_tag`, `trip_itineraries`, `trip_invites`, `trip_join_requests`, `trip_members`, `trip_queries`, `trip_reports`, `trip_wishlists`, `trip_rating`, `member_rating`, `host_rating`
5. `task_lock`
6. `conversation`
7. `conversation_participant`, `message`, `conversation_mute`, `conversation_block`
8. `user_notification`

---

## 6. Entity → table mapping summary

| Entity | Table |
|--------|--------|
| UsersEntity | users |
| UserProfileEntity | user_profile |
| UserProfileHistoryEntity | user_profile_history_table |
| RefreshTokenEntity | refresh_token |
| VerificationEntity | verification |
| AadharOtpEntity | aadhar_otp |
| SocialHandleEntity | social_handle |
| TravelPalEntity | travel_pal |
| UserReportEntity | user_reports |
| TripEntity | core_trip_details |
| TripPolicyEntity | trip_policies |
| TripMetaDataEntity | trip_meta_data |
| TagEntity | tags |
| (JoinTable) | trip_tag |
| TripItineraryEntity | trip_itineraries |
| TripInviteEntity | trip_invites |
| TripJoinRequestEntity | trip_join_requests |
| TripMemberEntity | trip_members |
| TripQueryEntity | trip_queries |
| TripReportEntity | trip_reports |
| TripWishlistEntity | trip_wishlists |
| TripRatingEntity | trip_rating |
| MemberRatingEntity | member_rating |
| HostRatingEntity | host_rating |
| TaskLockEntity | task_lock |
| ConversationEntity | conversation |
| ConversationParticipantEntity | conversation_participant |
| MessageEntity | message |
| ConversationMuteEntity | conversation_mute |
| ConversationBlockEntity | conversation_block |
| UserNotificationEntity | user_notification |
